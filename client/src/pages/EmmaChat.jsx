...
import { 
  MessageSquare, Settings, Sparkles, Send, RotateCcw, 
  Volume2, Compass, Award, Lightbulb, BookOpen, AlertCircle, HelpCircle, 
  User, CheckCircle, Mic, MicOff, ChevronDown, ChevronUp, X,
  ArrowLeft, Headphones, GraduationCap, Zap, Star
} from 'lucide-react';
...
const EmmaChat = () => {
  ...
  // Configuration State
  const [domain, setDomain] = useState('Vie quotidienne');
  ...
  const predefinedDomains = [
    { key: 'Vie quotidienne', label: 'Vie quotidienne', icon: '🏠', desc: 'Conversations de tous les jours' },
    { key: 'Voyage', label: 'Voyage & Tourisme', icon: '✈️', desc: 'Voyages et découvertes' },
    { key: 'Travail', label: 'Travail / Affaires', icon: '💼', desc: 'Monde professionnel' },
    { key: 'Entretien d\'embauche', label: 'Entretien d\'embauche', icon: '👔', desc: 'Recrutement et carrière' },
    { key: 'École / études', label: 'École & Études', icon: '🎓', desc: 'Vie étudiante' },
    { key: 'Informatique', label: 'Tech & Informatique', icon: '💻', desc: 'Technologie et digital' }
  ];

  const levelsList = [
    { code: 'A1', name: 'A1', fullName: 'Débutant', desc: 'Phrases très simples, vocabulaire basique', color: 'from-emerald-400 to-teal-500' },
    { code: 'A2', name: 'A2', fullName: 'Élémentaire', desc: 'Phrases courtes, questions simples', color: 'from-blue-400 to-cyan-500' },
    { code: 'B1', name: 'B1', fullName: 'Intermédiaire', desc: 'Échanges naturels, questions ouvertes', color: 'from-indigo-400 to-purple-500' },
    { code: 'B2', name: 'B2', fullName: 'Intermédiaire avancé', desc: 'Discussions fluides, opinions développées', color: 'from-purple-400 to-pink-500' },
    { code: 'C1', name: 'C1', fullName: 'Avancé', desc: 'Nuances linguistiques, vocabulaire précis', color: 'from-orange-400 to-red-500' },
    { code: 'C2', name: 'C2', fullName: 'Maîtrise', desc: 'Niveau natif, abstraction & débats', color: 'from-red-400 to-rose-500' }
  ];
  ...
  const toggleListening = async () => {
    ...
    try {
      ...
      rec.onerror = (e) => {
        ...
        setSpeechError(e.error || 'Erreur inconnue');
        setIsListening(false);
      };
      ...
      setSpeechError('Non autorisé ou erreur micro');
      setIsListening(false);
    }
  };
      ...
      mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          ...
          try {
            ...
            const res = await api.post('/student/chat/transcribe', { audioData: base64data, mimeType: 'audio/webm' });
            setUserInput(prev => prev + (prev ? " " : "") + res.data.text);
          } catch (err) {
            setSpeechError("Erreur de transcription audio.");
          } finally {
  ...
  const startConversation = async () => {
    ...
    if (!finalDomain.trim()) {
      alert("Veuillez saisir un domaine de conversation.");
      return;
    }
    ...
    try {
      const response = await api.post('/student/chat', {
        domain: finalDomain,
        level: level,
        message: "Bonjour Emma ! Présente-toi brièvement et commence notre conversation sur ce thème.",
        history: []
      });
      ...
      console.error(err);
      const errMsg = err.response?.data?.error || err.response?.data?.message || "Bonjour ! Je suis Emma. Je suis désolée, j'ai rencontré un problème pour me connecter. Peux-tu réessayer ?";
      setMessages([
        {
          id: 1,
          role: 'assistant',
  ...
  const sendMessage = async (e, textOverride = null) => {
    ...
    try {
      ...
      console.error(err);
      const errMsg = err.response?.data?.error || err.response?.data?.message || "Oups ! J'ai eu un petit problème technique pour te répondre. Peux-tu reformuler ton message ?";
      setMessages(prev => [
        ...prev,
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30" dir={isRtl ? 'rtl' : 'ltr'}>
                ...
                <div className="p-6 sm:p-10 space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-slate-900">Choisissez votre contexte</h3>
                        <p className="text-sm text-slate-500">Sélectionnez un thème de conversation</p>
                      </div>
                    </div>
                    ...
                    <input
                      type="text"
                      placeholder="Ex: Cuisine marocaine, Jeux vidéo, Sport..."
                      value={customDomain}
                      onChange={(e) => {
                        setDomain('custom');
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-slate-900">Votre niveau</h3>
                        <p className="text-sm text-slate-500">Sélectionnez votre niveau de français</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                      {levelsList.map((lvl) => (
                        ...
                        >
                          {level === lvl.code && (
                            <div className={`absolute inset-0 rounded-xl bg-gradient-to-br ${lvl.color} opacity-10`}></div>
                          )}
                          <div className="relative z-10">
                            <div className={`w-12 h-12 mx-auto rounded-xl flex items-center justify-center font-bold text-lg mb-2 transition-all ${
                              level === lvl.code
                            ...
                            <p className="text-sm font-semibold text-slate-900">{lvl.fullName}</p>
                            <p className="text-xs text-slate-500 mt-1 leading-tight">{lvl.desc}</p>
                          </div>
                        </button>
                      ))}
                    </div>
              ...
              <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-200/60 overflow-hidden min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h2 className="font-bold text-slate-900 text-sm sm:text-base truncate">Emma</h2>
                        <div className="flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
                          <p className="text-xs text-slate-500">En ligne • Professeure FLE</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => setShowMobileFeedback(!showMobileFeedback)}
                    ...
                    {messages.map((msg) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`flex flex-col max-w-[75%] sm:max-w-[70%] ${isAssistant ? '' : 'items-end'}`}>
                            <div className={`px-4 py-2.5 sm:px-5 sm:py-3 rounded-2xl text-sm leading-relaxed ${
                              isAssistant
                            ...
                            {isAssistant && (
                              ...
                              >
                                <Volume2 size={12} />
                                {speakingMessageId === msg.id ? 'Lecture...' : 'Écouter'}
                              </button>
                            )}
                            ...
                            {msg.role === 'user' && msg.feedback && (
                              ...
                              >
                                <Sparkles size={12} />
                                {expandedFeedbackIds[msg.id] ? 'Masquer' : 'Voir la correction'}
                              </button>
                            )}
                            ...
                            {expandedFeedbackIds[msg.id] && msg.feedback && (
                              ...
                              >
                                {msg.feedback.comprehension && (
                                  <div>
                                    <p className="font-semibold text-emerald-700 mb-1">Compréhension</p>
                                    <p className="text-slate-700">{msg.feedback.comprehension}</p>
                                  </div>
                                )}
                                {msg.feedback.reformulation && (
                                  <div>
                                    <p className="font-semibold text-indigo-700 mb-1">Reformulation</p>
                                    <p className="text-slate-700">{msg.feedback.reformulation}</p>
                                  </div>
                                )}
                                {msg.feedback.suggestion && (
                                  <div>
                                    <p className="font-semibold text-amber-700 mb-1">Suggestion</p>
                                    <p className="text-slate-700">{msg.feedback.suggestion}</p>
                                  </div>
                                )}
                              </motion.div>
                            )}
                ...
                <div className="flex-shrink-0 px-4 sm:px-6 py-3 sm:py-4 border-t border-slate-200 bg-white">
                  ...
                  <form onSubmit={sendMessage} className="flex items-center gap-2">
                    ...
                    <button
                      type="submit"
                      disabled={!userInput.trim() || loadingResponse || isTranscribing}
                ...
                <div className="flex-1 bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-200/60 overflow-hidden flex flex-col">
                  <div className="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-indigo-50 to-purple-50">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900">Feedback d'Emma</h3>
                        <p className="text-xs text-slate-500">Corrections en temps réel</p>
                      </div>
                    </div>
                  </div>
                    ...
                    {lastFeedback ? (
                      ...
                      >
                        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Votre message</p>
                          <p className="text-sm text-slate-700 italic">"{lastFeedback.userMessage}"</p>
                        </div>
                        ...
                        {lastFeedback.comprehension && (
                          <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                            ...
                            <p className="text-sm text-slate-700">{lastFeedback.comprehension}</p>
                          </div>
                        )}
                        ...
                        {lastFeedback.reformulation && (
                          <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                            ...
                            <p className="text-sm text-slate-700">{lastFeedback.reformulation}</p>
                          </div>
                        )}
                        ...
                        {lastFeedback.suggestion && (
                          <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                            ...
                            <p className="text-sm text-slate-700">{lastFeedback.suggestion}</p>
                          </div>
                        )}
                      </motion.div>
                    ) : (
                      <div className="h-full flex flex-col items-center justify-center text-center">
                        ...
                        <p className="font-semibold text-slate-700 mb-2">En attente</p>
                  ...
                  <div className="p-6">
                    ...
                    <ul className="space-y-3">
                      <li className="flex gap-3 text-sm text-slate-300">
                        ...
                        <span>Utilisez des phrases complètes pour un meilleur apprentissage</span>
                      </li>
                      <li className="flex gap-3 text-sm text-slate-300">
                        <span className="text-indigo-400 font-bold">2.</span>
                        <span>N'ayez pas peur des erreurs, Emma est là pour vous aider</span>
                      </li>
                      <li className="flex gap-3 text-sm text-slate-300">
                        <span className="text-indigo-400 font-bold">3.</span>
                        <span>Utilisez le microphone pour pratiquer votre prononciation</span>
                      </li>
                    </ul>
                  </div>
                </div>
                      ...
                      <div className="p-6 space-y-4">
                        {lastFeedback ? (
                          <>
                            ...
                            {lastFeedback.comprehension && (
                              <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                                <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wider mb-2">Compréhension</p>
                                <p className="text-sm text-slate-700">{lastFeedback.comprehension}</p>
                              </div>
                            )}
                            ...
                            {lastFeedback.reformulation && (
                              <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                                <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wider mb-2">Reformulation</p>
                                <p className="text-sm text-slate-700">{lastFeedback.reformulation}</p>
                              </div>
                            )}
                            ...
                            {lastFeedback.suggestion && (
                              <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                                <p className="text-xs font-semibold text-amber-700 uppercase tracking-wider mb-2">Suggestion</p>
                                <p className="text-sm text-slate-700">{lastFeedback.suggestion}</p>
                              </div>
                            )}
                          </>
                          ...
                          <div className="text-center py-8">
                            <p className="text-slate-500">Aucun feedback disponible pour le moment</p>
                          </div>
                        )}
                      </div>
                    </motion.div>
