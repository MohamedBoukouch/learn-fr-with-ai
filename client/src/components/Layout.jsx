...
import { 
  ...
  ShieldCheck, Users, BookMarked, FileCheck, HelpCircle, 
...
const Layout = ({ children }) => {
  ...
  const learnerItems = [
    ...
    { icon: <BookOpen size={20} />, label: t('learn_phrases'), path: '/dashboard/learn' },
    { icon: <MessageSquare size={20} />, label: t('chat_emma'), path: '/dashboard/chat' },
  ...
  const adminItems = [
    { icon: <LayoutDashboard size={20} />, label: t('dashboard'), path: '/admin/dashboard' },
    { icon: <BookMarked size={20} />, label: t('manage_content'), path: '/admin/content' },
    { icon: <HelpCircle size={20} />, label: t('manage_quizzes'), path: '/admin/quizzes' },
    { icon: <Users size={20} />, label: t('manage_users'), path: '/admin/users' },
    { icon: <FileCheck size={20} />, label: t('certificates'), path: '/admin/certificates' },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      >
        ...
        {/* Logo / Header */}
        <div className={`p-6 flex items-center gap-3 border-b border-slate-100 h-20 ${isCollapsed ? 'justify-center' : ''}`}>
        ...
        {/* User Card */}
        <div className={`p-4 border-b border-slate-100 flex items-center gap-3 ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isCollapsed && (
            <motion.div 
              ...
              className="min-w-0"
            >
              <h4 className="text-sm font-bold text-slate-950 truncate leading-snug">{userName}</h4>
      ...
      <div id="mobile-drawer" className="hidden fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm md:hidden">
        <div className="w-72 bg-white h-full flex flex-col text-slate-600 shadow-2xl ltr:border-r rtl:border-l border-slate-200">
          ...
          <div className="p-6 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-slate-900 leading-snug">{userName}</h4>
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                {isAdmin ? t('admin') : t('learner')}
              </span>
            </div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${
                    isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/10' : 'hover:bg-slate-50 text-slate-600 hover:text-slate-900'
                  }`
                }
              >
                {item.icon}
